...
def send_task_completion_email(doc):
	...
	if doc.status == "Completed" and doc.completed_sent == 0:
		...
		helpdesk_email_body = f"""Task ticket number {doc.name}, {doc.customer} has been successfully completed."""
...
def send_task_assignment_email(task):
	if task.completed_by:
		...
		body = f"""Ticket no. {task.issue}, {task.customer} has been assigned to our Engineer {task.technician_name}"""
		recipients = frappe.db.get_value("Company", task.company, "support_email")
		if task.type_of_call == "Toner":
			body = f"""Ticket no. {task.issue}, {task.customer} with type of call "Toner" has been assigned to our Engineer {task.technician_name}"""
